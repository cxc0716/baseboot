...
public class WeixinServiceImpl implements WeixinService {
	...
	public QrcodeInfo getQrcodeInfo() {
		...
		try {
			...
			String uuid = uuidResult.substring(uuidResult.indexOf("\"") + 1, uuidResult.lastIndexOf("\""));
			String qrcodeUrl = "https://login.weixin.qq.com/qrcode/" + uuid;
			QrcodeInfo qrcodeInfo = new QrcodeInfo();
			qrcodeInfo.setUuid(uuid);
			qrcodeInfo.setQrcode(qrcodeUrl);
			if (logger.isDebugEnabled()) {
				logger.debug("qrcode request:{}", JacksonUtil.write(qrcodeInfo));
			}
			return qrcodeInfo;
		} catch (IOException e) {
	...
	public Boolean sendMsg(String uuid, Content content) throws IOException {
		...
		String redirectUrl = s2.substring(s2.indexOf("\"") + 1, s2.lastIndexOf("\""));
		...
		ContactResponse contactResponse = JSON.parseObject(s5, ContactResponse.class);
	...
	private List<Contact> filterContact(List<Contact> contacts, Content content) {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String s6 = httpClientTemplate.executePost(sendMsg, JSON.toJSONString(baseRequest), "utf-8");
	...
	private boolean sendSingleMsg(String passTicket, PostBody body) {
		try {
			String sendMsg = "https://wx.qq.com/cgi-bin/mmwebwx-bin/webwxsendmsg?lang=zh_CN&pass_ticket=%s";
			sendMsg = String.format(sendMsg, passTicket);
			String s6 = httpClientTemplate.executePost(sendMsg, JacksonUtil.write(body), "utf-8");
